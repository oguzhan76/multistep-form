...
export default function PlanForm({ offers, plan, onChange }: PlanFormProps) {
    ...
    const handleRecurringToggle = (e: React.ChangeEvent<HTMLInputElement>) => {
        if (e.target.checked)
            onChange({ recurring: Recurring.YEARLY });
        else
            onChange({ recurring: Recurring.MONTHLY });
    }
        ...
        <div className="px-6 py-6 ds:pl-16 ds:pr-10 ds:pt-0 ds:pb-6">
            ...
            <section className='ds:mt-10 w-full flex flex-col ds:flex-row gap-3 ds:gap-5 text-marine_blue'>
                ...
                >
                    ...
                    <div className='ml-4 ds:ml-0 ds:mt-10 flex flex-col items-start'>
                        ...
                        <p className='font-thin text-cool_gray text-sm'>{'$' + offers.arcade[plan.recurring] + (plan.recurring === 'monthly' ? '/mo' : '/yr')}</p>
                        {plan.recurring === 'yearly' && <p className='pt-1 text-xs font-medium font'>2 months free</p>}
                    </div>
                </button>
                <button
                ...
                >
                    ...
                    <div className='ml-4 ds:ml-0 ds:mt-10 flex flex-col items-start'>
                        ...
                        <p className='font-thin text-cool_gray text-sm'>{'$' + offers.advanced[plan.recurring] + (plan.recurring === 'monthly' ? '/mo' : '/yr')}</p>
                        {plan.recurring === 'yearly' && <p className='pt-1 text-xs font-medium font'>2 months free</p>}
                    </div>
                </button>
                <button
                ...
                >
                    ...
                    <div className='ml-4 ds:ml-0 ds:mt-10 flex flex-col items-start'>
                        ...
                        <p className='font-thin text-cool_gray text-sm'>{'$' + offers.pro[plan.recurring] + (plan.recurring === 'monthly' ? '/mo' : '/yr')}</p>
                        {plan.recurring === 'yearly' && <p className='pt-1 text-xs font-medium font'>2 months free</p>}
                    </div>
                </button>
            </section>
            ...
            <section>
                <div className="text-marine_blue flex bg-magnolia mt-6 ds:mt-10 justify-center h-12 items-center rounded-xl">
                    <p className={plan.recurring !== Recurring.MONTHLY ? 'text-cool_gray' : ''}>Monthly</p>
                    <div className='mx-4 flex justify-center items-center'>
                        <input
                            id='toggle'
                            className='w-0 h-0 invisible peer'
                            type="checkbox"
                            onChange={(e) => handleRecurringToggle(e)}
                            checked={plan.recurring === Recurring.YEARLY}
                        />
                        <label
                    ...
                    <p className={plan.recurring === Recurring.MONTHLY ? 'text-cool_gray' : ''}>Yearly</p>
                </div>
            </section>
        </div>
    )
